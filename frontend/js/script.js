...
async function AdicionarCadastro() {
    ...
    if (nomeCadastro === '' || nickname === '' || emailCadastro === '' || senhaCadastro === '' || telCadastro === '') {
        ...
        try {
            fetch('http://localhost:5195/api/Cadastro/Cadastro', {
                method: 'POST',
                headers: {
...
async function EntrarCadastro() {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            const response = await fetch(`http://localhost:5195/api/Cadastro/Cadastro/BuscarPorNick?nick=${nickname}`);
...
async function CriarRegistro(nickname) {
    ...
    try {
        ...
        try {
            ...
            fetch('http://localhost:5195/api/RegistroDePartida', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
...
async function RegistroPart() {
    ...
    try {
        ...
        console.log(cadastroId)
        const response = await fetch(`http://localhost:5195/api/RegistroDePartida/${cadastroId}`);
        const data = await response.json();
        console.log(data)
        console.log(response)
...
async function exibirRanking() {
    ...
    try {
        const response = await fetch('http://localhost:5195/api/Ranking');
        const data = await response.json();
